...
private:
  ...
  // L'HTML della pagina web (salvato come stringa raw)
...
<head>
  ...
  <style>
    ...
    .btn-kill { background-color: #6cd9ff; color: #fff; }
...
<style>
  ...
  /* Il contenitore che mette i bottoni sulla stessa riga */
  .pres-container {
    display: flex;         /* Attiva il layout in linea */
    justify-content: center; /* Centra i bottoni nella riga */
    gap: 15px;             /* Spazio tra i due bottoni */
    margin-top: 15px;
  }

  /* Stile estetico per i bottoni */
  .btn.slide-btn {
    padding: 10px 20px;
    font-size: 16px;
    cursor: pointer;
    ...
    flex: 1; /* Fa sì che i bottoni abbiano la stessa larghezza */
    max-width: 150px;
  }
...
public:
  ...
  void begin() {
    // 1. Inizializza LED
    led.begin();
    led.setBrightness(50); // Luminosità moderata
    led.setPixelColor(0, led.Color(0, 0, 0)); // Spento all'avvio
    led.show();

    // 2. Configura Access Point con LIMITE (max_connection = 1)
  ...
  void updateLED() {
    ...
    if (numStations > 0) {
      // Almeno un device connesso -> VERDE
      led.setPixelColor(0, led.Color(255, 0, 127)); 
    } else {
      // Nessun device -> SPENTO (o un altro colore a tua scelta)
  ...
  void handle() {
    server.handleClient();
    updateLED(); // Aggiorna lo stato del LED ad ogni ciclo
  }
};
